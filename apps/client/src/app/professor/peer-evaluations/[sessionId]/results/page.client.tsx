...
import type { PeerEvaluationSessionResponse, PeerEvaluationSessionResultsResponse } from '@/lib/types';

interface ProfessorPeerEvaluationsResultsPageClientProps {
  sessionId: number;
}
...
export default function ProfessorPeerEvaluationsResultsPageClient({
  sessionId,
}: ProfessorPeerEvaluationsResultsPageClientProps) {
  ...
  const [results, setResults] = useState<PeerEvaluationSessionResultsResponse | null>(null);
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const [sessionResponse, resultsResponse] = await Promise.all([
        ...
        peerEvaluationsApi.getResults(sessionId),
      ]);
