...
import type {
  PeerEvaluationParsePreviewMember,
  PeerEvaluationParseUnresolvedItem,
  PeerEvaluationSessionResponse,
} from '@/lib/types';

interface ProfessorPeerEvaluationsEditPageClientProps {
  sessionId: number;
}
...
type UnresolvedSelection = {
  ...
  reason: string;
  candidates: PeerEvaluationParseUnresolvedItem['candidates'];
};
...
function flattenTeams(teams: Record<string, PeerEvaluationParsePreviewMember[]>): PeerEvaluationParsePreviewMember[] {
  return Object.values(teams).flat();
}

function buildCanonicalRawText(members: PeerEvaluationParsePreviewMember[]): string {
...
export default function ProfessorPeerEvaluationsEditPageClient({
  sessionId,
}: ProfessorPeerEvaluationsEditPageClientProps) {
  ...
  const [parsedMembers, setParsedMembers] = useState<PeerEvaluationParsePreviewMember[]>([]);
  ...
  const loadPage = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await peerEvaluationsApi.getSession(sessionId);
      setSession(response);
  ...
  const resolvedUnresolvedMembers = useMemo(() => {
    return unresolvedSelections
      .map((item): PeerEvaluationParsePreviewMember | null => {
      ...
      .filter((member): member is PeerEvaluationParsePreviewMember => member !== null);
  ...
  const candidateOptionsByKey = useMemo(() => {
    ...
    const options = new Map<string, PeerEvaluationParseUnresolvedItem['candidates']>();
    for (const item of unresolvedSelections) {
      options.set(
        item.key,
  ...
  const handleParseMembers = useCallback(async () => {
    ...
    try {
      const parsed = await peerEvaluationsApi.parseMembers(
        sessionId,
      ...
      setUnresolvedSelections(
        parsed.unresolved_members.map((item) => ({
          key: `${item.team_label}::${item.raw_name}`,
          team_label: item.team_label,
          raw_name: item.raw_name,
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    try {
      const updatedSession = await peerEvaluationsApi.updateSession(sessionId, {
        title: normalizedTitle,
      });

      await peerEvaluationsApi.confirmMembers(sessionId, {
          ...
          actions={
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                disabled={!canSave}
                data-testid="peer-eval-edit-save-all"
              >
                {saving ? '저장 중...' : '저장'}
              </Button>
            </div>
            ...
            <Card className="glass-card rounded-xl animate-entrance border-0 shadow-md">
              ...
              <CardContent className="space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="peer-eval-title">세션 제목</Label>
                  <Input
                    id="peer-eval-title"
                    value={title}
                    ...
                    placeholder="세션 제목"
                    disabled={isBusy}
                    data-testid="peer-eval-edit-title-input"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="peer-eval-raw-members">팀 구성 원문</Label>
                  <Textarea
                    id="peer-eval-raw-members"
                    value={rawText}
                    onChange={(event) => {
                      setCanonicalRawText(null);
                    ...
                    className="min-h-36"
                    disabled={isBusy}
                    data-testid="peer-eval-edit-raw-input"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    disabled={saving}
                    data-testid="peer-eval-edit-parse-button"
                  >
                    {parsingMembers ? (
                  ...
                  {parsingMembers ? (
                    <Button type="button" variant="outline" onClick={handleCancelParse} data-testid="peer-eval-edit-parse-cancel">
                      취소
                    </Button>
                  ) : null}
                </div>
            ...
            <Card className="glass-card rounded-xl animate-entrance border-0 shadow-md">
              ...
              <CardContent className="space-y-4">
                ...
                {unresolvedSelections.length > 0 ? (
                  <div className="space-y-3" data-testid="peer-eval-edit-unresolved-list">
                ...
                {multiTeamWarnings.length > 0 ? (
                  <Alert variant="destructive" data-testid="peer-eval-edit-duplicate-team-warning">
                    <AlertDescription>
                      <div className="space-y-2">
                ...
                {parseCompleted && warningsResolved ? (
                  <div
                    ...
                    data-testid="peer-eval-edit-parse-clean"
                  >
