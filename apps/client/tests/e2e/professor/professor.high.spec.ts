...
function ensureServerSchemaReady(): void {
  ...
  const script = [
    ...
    '        await conn.execute(',
    '            text(',
    "                \"ALTER TABLE comments ADD COLUMN IF NOT EXISTS comment_type VARCHAR(16) NOT NULL DEFAULT 'peer'\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"UPDATE comments SET comment_type = 'peer' WHERE comment_type IS NULL OR comment_type = ''\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_comments_comment_type ON comments(comment_type)\"",
    '            )',
    '        )',
    '',
    '        await conn.execute(',
    ...
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_user_id ON student_risk_snapshots(user_id)\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_evaluated_at ON student_risk_snapshots(evaluated_at)\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_risk_band ON student_risk_snapshots(risk_band)\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_risk_score ON student_risk_snapshots(risk_score)\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_user_evaluated_at ON student_risk_snapshots(user_id, evaluated_at)\"",
    '            )',
    '        )',
    '        await conn.execute(',
    '            text(',
    "                \"CREATE INDEX IF NOT EXISTS ix_student_risk_snapshots_band_score_evaluated_at ON student_risk_snapshots(risk_band, risk_score, evaluated_at)\"",
    '            )',
    '        )',
    '',
