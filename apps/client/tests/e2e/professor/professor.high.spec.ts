...
function ensureServerSchemaReady(): void {
  ...
  const script = [
    ...
    '',
    'def _ensure_comment_type_column(sync_conn) -> None:',
    '    inspector = inspect(sync_conn)',
    '    columns = inspector.get_columns("comments")',
    '    has_comment_type = any(col.get("name") == "comment_type" for col in columns)',
    '    if has_comment_type:',
    '        return',
    '',
    ...
    '    StudentRiskSnapshot.__table__.create(sync_conn, checkfirst=True)',
    '    _ensure_comment_type_column(sync_conn)',
    '',
    '    has_untyped_comment = sync_conn.execute(',
    '        text("SELECT 1 FROM comments WHERE comment_type IS NULL OR comment_type = \'\' LIMIT 1")',
    '    ).first()',
    '    if has_untyped_comment:',
    '        sync_conn.execute(',
    '            text("UPDATE comments SET comment_type = \'peer\' WHERE comment_type IS NULL OR comment_type = \'\'")',
    '        )',
    '',
    '    _ensure_model_indexes(sync_conn, Comment)',
    '    _ensure_model_indexes(sync_conn, StudentRiskSnapshot)',
    '',
    'async def main() -> None:',
