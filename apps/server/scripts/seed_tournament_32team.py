"""
32팀 × 5명, 32강 패자부활전, allow_self_vote=True 토너먼트 세션 시드 스크립트.
...
SESSION_TITLE = "32강 패자부활전 테스트 (팀원 투표 허용)"
NUM_TEAMS = 32
MEMBERS_PER_TEAM = 5
...
async def ensure_students(db):
    ...
    for i in range(1, total + 1):
        email = f"tour32-student-{i:03d}@example.com"
        name = f"선수{i:03d}"
    ...
    refreshed = []
    for i in range(1, total + 1):
        user = await crud.get_user_by_email_basic(db, f"tour32-student-{i:03d}@example.com")
        if user is None:
