...
def _patch_form_context(monkeypatch, *, is_open: bool = True, team_user_ids=None):
    ...
    async def fake_get_session_by_access_token(_db, _token):
        return SimpleNamespace(id=22, is_open=is_open)
    ...
    monkeypatch.setattr(peer_evaluations.peer_eval_crud, "list_team_member_users", fake_list_team_member_users)


def test_submit_peer_evaluation_form_rejects_closed_session(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
    ...
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(
            inspect.unwrap(peer_evaluations.submit_peer_evaluation_form)(
                token="token",
                payload=payload,
                request=request,
                db=object(),
...
def test_submit_peer_evaluation_form_rejects_count_mismatch(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
...
def test_submit_peer_evaluation_form_rejects_outside_team_evaluatee(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
    _patch_form_context(monkeypatch, team_user_ids=[101, 102])
...
def test_submit_peer_evaluation_form_rejects_duplicate_evaluatee(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
    _patch_form_context(monkeypatch, team_user_ids=[101, 102])
...
def test_submit_peer_evaluation_form_rejects_invalid_total(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
    _patch_form_context(monkeypatch, team_user_ids=[101, 102])
...
def test_submit_peer_evaluation_form_success(monkeypatch):
    request = _make_request("/peer-evaluations/forms/token/submit")
    _patch_form_context(monkeypatch, team_user_ids=[101, 102])
    ...
    monkeypatch.setattr(peer_evaluations.peer_eval_crud, "upsert_submission_entries", fake_upsert_submission_entries)
